...
tournamentsRouter.get('/', async (req: Request, res: Response): Promise<void> => {
  const { leagueId } = req.params;
  const { status } = req.query;

  try {
    let query = db.collection(`leagues/${leagueId}/tournaments`) as FirebaseFirestore.Query;
    if (status) {
    ...
    const tournaments = snap.docs.map((d) => ({ id: d.id, ...serializeDoc(d.data()) }));
...
tournamentsRouter.get('/:tId', async (req: Request, res: Response): Promise<void> => {
  const { leagueId, tId } = req.params;
  try {
    const snap = await db.doc(`leagues/${leagueId}/tournaments/${tId}`).get();
    if (!snap.exists) { res.status(404).json({ error: 'Tournament not found' }); return; }
...
tournamentsRouter.post('/', authenticate, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const docRef = await db.collection(`leagues/${leagueId}/tournaments`).add({
      ...data,
...
tournamentsRouter.patch('/:tId', authenticate, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  const { leagueId, tId } = req.params;
...
tournamentsRouter.delete('/:tId', authenticate, requireAdmin, async (req: Request, res: Response): Promise<void> => {
  const { leagueId, tId } = req.params;
  try {
